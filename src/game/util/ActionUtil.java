	...
	public static PossibleActions getPossibleActions(Game game, Unit unit) {
		...
		while (!open.isEmpty()) {
			...
			for (Direction d : Direction.values()) {
				...
				if (!field.isAccessible() || distance > unit.getType().getMovementDistance() + unit.getType().getMaxAttackDistance() || (u.isPresent() && u.get().getPlayer() != unit.getPlayer()))
					continue;

				if (!unit.getType().isFlying() && !(field.isWaterTile() && unit.getType().isSwimming()) && !(!field.isWaterTile()) && unit.getType().isWalking())
					continue;
